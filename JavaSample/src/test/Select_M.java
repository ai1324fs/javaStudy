...
public class Select_M extends Access_M implements printProcess {
	...
	public void select_accessProcess() {
		...
		System.out.println("회원번호\t이름\t비밀번호\t성별\t재산\t키\t몸무게\t나이\t급여\t운동여부\t음주여부\t흡연여부");
		System.out.println("---------------------------결혼 정보 시스템-----------------------------------------------------------");
		...
		try {
			while(rs.next()) {
				...
				int propertym = rs.getInt(5);
			    double height = rs.getDouble(6);
			    double weight = rs.getDouble(7); 
			    int age = rs.getInt(8);
			    int paym = rs.getInt(9); 
			    String exercise = rs.getString(10);
			    String drink = rs.getString(11);
			    String smoking = rs.getString(12);
			  
				
			    
			   System.out.println(no+"\t"+pw+"\t"+name+"\t"+gender+"\t"+propertym+"\t"+height+"\t"+weight+"\t"+age+"\t"+paym+"\t"+exercise+"\t"+drink+"\t"+smoking);
			}
			System.out.println("-------------------------------------------------------------------------------------------------");
		} catch (SQLException e) {
			e.printStackTrace();
